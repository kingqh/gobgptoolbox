...
func UnmarshalRD(rd *any.Any) (bgp.RouteDistinguisherInterface, error) {
	...
	if err := ptypes.UnmarshalAny(rd,
		&rdValue); err != nil {
		return nil, fmt.Errorf("failed to unmarshal route distinguisher with error: %+v",
			err)
	}

	switch v := rdValue.Message.(type) {
	case *api.RouteDistinguisherTwoOctetASN:
		return bgp.NewRouteDistinguisherTwoOctetAS(uint16(v.Admin),
			v.Assigned), nil
	case *api.RouteDistinguisherIPAddress:
		return bgp.NewRouteDistinguisherIPAddressAS(v.Admin,
			uint16(v.Assigned)), nil
	case *api.RouteDistinguisherFourOctetASN:
		return bgp.NewRouteDistinguisherFourOctetAS(v.Admin,
			uint16(v.Assigned)), nil
	default:
		return nil, fmt.Errorf("Unknown route distinguisher type: %+v",
			v)
	}
}
...
func UnmarshalRT(rts []*any.Any) ([]bgp.ExtendedCommunityInterface, error) {
	repl := make([]bgp.ExtendedCommunityInterface,
		0)
	for i := 0; i < len(rts); i++ {
		var rtValue ptypes.DynamicAny
		if err := ptypes.UnmarshalAny(rts[i],
			&rtValue); err != nil {
			return nil, fmt.Errorf("failed to unmarshal route target with error: %+v",
				err)
		}
		switch v := rtValue.Message.(type) {
		case *api.TwoOctetAsSpecificExtended:
			repl = append(repl,
				bgp.NewTwoOctetAsSpecificExtended(bgp.ExtendedCommunityAttrSubType(v.SubType),
					uint16(v.Asn),
					v.LocalAdmin,
					v.IsTransitive))
		case *api.IPv4AddressSpecificExtended:
			repl = append(repl,
				bgp.NewIPv4AddressSpecificExtended(bgp.ExtendedCommunityAttrSubType(v.SubType),
					v.Address,
					uint16(v.LocalAdmin),
					v.IsTransitive))
		case *api.FourOctetAsSpecificExtended:
			repl = append(repl,
				bgp.NewFourOctetAsSpecificExtended(bgp.ExtendedCommunityAttrSubType(v.SubType),
					v.Asn,
					uint16(v.LocalAdmin),
					v.IsTransitive))
		default:
			return nil, fmt.Errorf("Unknown route target type: %+v",
				v)
		}
	}
		...
		case 1:
			for _, stlv := range tlv {
				...
				o.Sid = make([]byte,
					16)
				copy(o.Sid,
					[]byte(net.ParseIP(infoS.SID).To16()))
...
func UnmarshalPrefixSID(svcs []*any.Any) (*prefixsid.PSid, error) {
	...
	for _, svc := range svcs {
		...
		if err := ptypes.UnmarshalAny(svc,
			&svcValue); err != nil {
			return nil, fmt.Errorf("failed to unmarshal Prefix SID with error: %+v",
				err)
		}
...
func UnmarshalSRv6SubTLVs(stlvs map[uint32]*api.SRv6TLV) (map[uint8][]srv6.SubTLV, error) {
	var err error
	mtlvs := make(map[uint8][]srv6.SubTLV,
		len(stlvs))
	for t, tlv := range stlvs {
		switch t {
		case 1:
			for _, stlv := range tlv.Tlv {
				...
				if err := ptypes.UnmarshalAny(stlv,
					&stlvValue); err != nil {
					return nil, fmt.Errorf("failed to unmarshal SRv6 SID Structure Sub Sub TLV with error: %+v",
						err)
				}
...
func UnmarshalSRv6SubSubTLVs(stlvs map[uint32]*api.SRv6TLV) (map[uint8][]srv6.SubSubTLV, error) {
	mtlvs := make(map[uint8][]srv6.SubSubTLV,
		len(stlvs))
	for t, tlv := range stlvs {
		switch t {
		case 1:
			for _, sstlv := range tlv.Tlv {
				...
				if err := ptypes.UnmarshalAny(sstlv,
					&sstlvValue); err != nil {
					return nil, fmt.Errorf("failed to unmarshal SRv6 SID Structure Sub Sub TLV with error: %+v",
						err)
				}
				...
				if !ok {
					sstlvs = make([]srv6.SubSubTLV,
						0)
				}
				sstlvs = append(sstlvs,
					e)
...
func MarshalRDFromString(rd string) (*any.Any, error) {
	...
	if net.ParseIP(parts[0]).To4() != nil {
		...
		return MarshalRD(bgp.NewRouteDistinguisherIPAddressAS(parts[0],
			uint16(n))), nil
	}
	...
	if n1 < math.MaxUint16 {
		...
		return MarshalRD(bgp.NewRouteDistinguisherTwoOctetAS(uint16(n1),
			uint32(n2))), nil
	}
	// Since no match before then, it is 4 Bytes ASN: 2 Bytes Value
	return MarshalRD(bgp.NewRouteDistinguisherFourOctetAS(uint32(n1),
		uint16(n2))), nil
}
...
func MarshalRTFromString(rt string) (*any.Any, error) {
	...
	if net.ParseIP(parts[0]).To4() != nil {
		// If parts[0] is a valid IP, then it is IP:Value
		n, _ := strconv.Atoi(parts[1])
		return MarshalRT(bgp.NewIPv4AddressSpecificExtended(bgp.EC_SUBTYPE_ROUTE_TARGET,
			parts[0],
			uint16(n),
			true)), nil
	}
	...
	if n1 < math.MaxUint16 {
		// If parts[0] is less than MaxUint16, then it is 2 Bytes ASN: 4 Bytes Value
		return MarshalRT(bgp.NewTwoOctetAsSpecificExtended(bgp.EC_SUBTYPE_ROUTE_TARGET,
			uint16(n1),
			uint32(n2),
			true)), nil
	}
	...
	// Since no match before then, it is 4 Bytes ASN: 2 Bytes Value
	return MarshalRT(bgp.NewFourOctetAsSpecificExtended(bgp.EC_SUBTYPE_ROUTE_TARGET,
		uint32(n1),
		uint16(n2),
		true)), nil
}
